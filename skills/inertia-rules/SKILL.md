...
- [design-system/styling.md](rules/design-system/styling.md) — `ds-` tokens **and arrangement utilities** live in `ui` primitives; consumers pass variant props and use layout primitives' props (no raw `flex`/`grid`/`gap`); `className` in app code limited to a positioning allow-list; the decision procedure.
- [design-system/components.md](rules/design-system/components.md) — primitive/app/feature roles; `ui` owns tokens **and all raw HTML** (features/pages/layouts render only components), `app` is token-free (bar the `NavLink` exception); anticipatory + domain-free promotion; appearance-via-variants, arrangement-via-layout-primitives; the no-raw-HTML rule + its forbid-elements/`className` enforcement.
